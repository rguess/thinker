...
}, $.validator.format("登录名必须是以字母开头，长度在5到15之间"));

//密码验证
jQuery.validator.addMethod("isPasswd", function(value, element, param) {
	var reg =/[\x21-\x7E]{6,15}/;
	 return reg.test(value);
}, $.validator.format("密码长度必须在6-15之间"));
...
jQuery.validator.addMethod("isNotChinese", function(value, element, param) {
	 var reg =/^[^\u4e00-\u9fa5]{0,}$/;
	 return reg.test(value);
}, $.validator.format("不能含有中文"));

//不能有空格
jQuery.validator.addMethod("isNotSpace", function(value, element, param) {
	 var reg =/^\S+$/gi;
	 return reg.test(value);
}, $.validator.format("不能有空格"));